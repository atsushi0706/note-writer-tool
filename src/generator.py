...
"""
    ...
    system_prompt = f"""あなたは**読者の感情を動かす**プロのコピーライターです。
タイトルと本文を、ONE HACK（H→A→C→K）の構成で書いてください。
...
【絶対ルール】
- H→A→C→Kの流れを守る。ラベル（H/A/C/K等）は本文に出さない
- ペルソナに書かれた職業名・属性をそのまま本文に登場させる（汎用語に丸めない）
- 読者が「これ、私のことだ」と感じる具体場面を入れる
- 感情を動かすこと最優先。論文や教科書のような説明は避ける
"""
