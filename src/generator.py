...
【★引用の絶対ルール（重要）】
- **専門家・研究者の引用は最大2人まで**。3人以上引用したら教科書になる
- 「〇〇博士は〜と言いました」式の羅列は厳禁
- 1人の専門家の知見を**深く掘り下げる**方が、3人を浅く紹介するより良い
- 「ResearchGateの研究では…」のような出典名の連呼は禁止

【★比喩の絶対ルール（重要）】
- **使い古された比喩を避ける**。以下は特に禁止:
  - シェフ／レストラン／屋台／料理人（ビジネス書で使い古された）
  - スポーツ選手のメタファー（マラソン・野球など、誰でも思いつく）
  - 旅・登山（ありきたり）
- 比喩は**読者の日常の具体場面**から取る:
  - 化粧台での身支度／キッチンの片付け／電車のホーム／コンビニのレジ
  - 子どもの寝顔／植物の水やり／玄関の靴の並び
- **その比喩じゃないと言えないこと**だけ比喩にする。装飾の比喩は禁止

【★アクション提案の絶対ルール】
- **「〇〇ファイル作成」「〇〇ノート」「振り返りジャーナル」は禁止**（汎用すぎ）
- 読者が**今日の生活の中で5分で試せる**具体的な動作にする
- ペルソナの実際の場面に紐づくこと（コーチなら「次のセッション前に〜する」など）

【★文体】
- 学術論文でも教科書でもない。**友人がカフェで話してくれる感じ**
- 引用が多いと冷たくなる。引用は控えて自分の言葉で語る部分を増やす

【★ペルソナを記事に必ず反映する（最重要）】
- ペルソナの**職業名・属性を本文に明示的に登場させる**
  - 例: ペルソナが「コーチ」なら、本文に「コーチ」という言葉を3回以上出す
  - 「ビジネスをしている人」「専門職」のような汎用語に丸めない
- ペルソナの**実際の仕事場面・典型的な瞬間を1つ以上必ず描写する**
  - コーチなら: 体験セッション、継続契約の提案、Zoom面談、フィードバックシート、月額料金の提示
  - ヒーラーなら: 個人セッション、エネルギーワーク、ヒーリング後のクライアント反応
  - 経営者なら: 役員会、決算、社員面談、クライアント商談
- ペルソナと無関係な誰にでも当てはまる文章は禁止
- 読者が「これ、私のために書かれてる」と感じる**具体場面の描写**を必ず入れる
"""
