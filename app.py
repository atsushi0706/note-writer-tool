...
with st.expander("📖 はじめての方へ｜このツールの使い方", expanded=True):
    st.markdown("""
### 🎯 このツールでできること
あなたのプロフィールと書きたいテーマから、**ONE HACK構成**でnote記事を自動生成します。

- ✅ 海外のエビデンス・専門家の知見を**実際にWeb検索**してリサーチ
- ✅ AIと相談しながらコンセプトを練り上げる
- ✅ 「進め方プラン」を確認してから本文生成（外れない仕上がり）
- ✅ プロフィール・過去の痛みを記事に反映
- ✅ Instagram・LP等への誘導文も自然に挿入
...
| Step | 内容 |
|---|---|
| **① 入力** | プロフィール → コンセプト（AI相談 or 自分で入力） → ペルソナ → オプション設定 |
| **② リサーチ + プラン確認** | AIが海外エビデンスをリサーチ → 進め方プランを提案 → 確認 |
| **③ 記事完成** | 2,000字の記事が完成 → コピーしてnoteに貼り付け |
...
2. APIキーをまだ持っていない方は、[**APIキー取得マニュアル**](https://github.com/atsushi0706/note-writer-tool/blob/master/docs/GEMINI_API_KEY_GUIDE.md)を参照してください
...
- **過去の痛み**を書くと、AIがその痛みをペルソナの痛みに重ねて深い記事になる
- コンセプトに迷ったら **AI相談モード** を使う → チャットで「もっと女性向けに」など指示可
...
このツールは **下書き保存までは自動化していません**。理由:
- noteへの自動ログインはセキュリティ上避けたほうが良い

→ 完成した記事は「コピー → note新規作成画面に貼り付け」してください。
ボタン1つでnoteの新規作成ページが開けます。
""")
    st.caption("このパネルは ▼ をクリックして閉じられます。")
