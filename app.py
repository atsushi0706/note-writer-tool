...
with st.expander("📖 はじめての方へ｜このツールの使い方", expanded=True):
    st.markdown("""
### 🚨 まず最初にやること（これがないと何も動きません）

**左サイドバーに「Gemini API Key」を貼り付けてください。**

- API Keyとは: **AIに話しかけるための鍵**（Googleアカウントがあれば無料でもらえる）
- クレジットカード登録 **不要**
- 取得手順マニュアル → [**APIキー取得・設定マニュアル**](https://github.com/atsushi0706/note-writer-tool/blob/master/docs/GEMINI_API_KEY_GUIDE.md)

> ⚠️ **API Keyを入れないと、リサーチも記事生成もボタンが押せません。** 一番最初に貼ってから始めてください。

---

### ⚠️ 1日に書ける記事数の目安

無料プランには「1日にAIを呼べる回数」の上限があります（人によって上限が違います）。

- このツールは1記事を作るのにAIを **2〜3回** 呼びます
- ざっくり **1日 数本〜10本程度** が現実的な目安
- 「使い切った」と出たら **24時間待つ** か、**別のGmailで新しい鍵** を作る
- 正確な上限 → [Gemini レート制限ページ](https://ai.google.dev/gemini-api/docs/rate-limits)

---

### 🎯 このツールでできること

あなたのプロフィールと書きたいテーマから、**読まれるnote記事**を自動で書き上げます。
...
- **プロフィールは具体的に書く**
  - ❌「キャリアコーチ」 → ✅「元銀行員のキャリアコーチ。30代女性向けに副業から起業を支援」
- **過去の痛みを書く** と、ペルソナの痛みに重ねて深い記事になる
- **コンセプトはなるべく自分で書く** とAPIを節約できる（AI相談はAIを1回多く呼ぶ）
- 進め方プランの段階で違和感があれば、チャットで修正してから記事生成へ
...
「🚀 noteで新規作成」ボタンで、noteの新規作成ページがすぐ開けます。
""")
